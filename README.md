...
## Overview
AKiPa is a python script aimed to analyze pre-steady state kinetic data obtained from analysis of stopped-flow data (KiPaD), namely $k_{obs}$ at different ligand conncentrations ([L]), to determine the interaction and kinetic parameters of the observed process: dissociation constant($K_d$) and limiting rate constant for the the reactive process ($k_{lim}$)
